...
void TanhLayer<DataType>::act_fun(Matrix<DataType, 2> & t, const Matrix<DataType, 2> & s) {
    t = single_op<tanh_map<DataType>>(s);
}

template<typename DataType>
void TanhLayer<DataType>::act_fun_de(Matrix<DataType, 2> & t, const Matrix<DataType, 2> & s) {
    t = 1 - s * s;
}

template<typename DataType>
void TanhLayer<DataType>::forward_cpu(const vector<Blob<DataType> *> & input_blob,
...
template<typename DataType>
void TanhLayer<DataType>::backward_cpu(const vector<Blob<DataType> *> & input_blob,
