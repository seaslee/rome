...
void RELULayer<DataType>::act_fun(Matrix<DataType, 2> & t, const Matrix<DataType, 2> & s) {
    t = single_op<rectfier<DataType>>(s);
}

template<typename DataType>
void RELULayer<DataType>::act_fun_de(Matrix<DataType, 2> & t, const Matrix<DataType, 2> & s) {
    t = single_op<rectfier_deri_map<DataType>>(s);
}

template<typename DataType>
void RELULayer<DataType>::forward_cpu(const vector<Blob<DataType> *> & input_blob,
...
template<typename DataType>
void RELULayer<DataType>::backward_cpu(const vector<Blob<DataType> *> & input_blob,
